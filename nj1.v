...
- rewrite <- (app_nil_l _) in pi1.
  ...
  + eapply (snd (fst (IHm _ Hpi _ eq_refl)) _ _ _ n) in pi1...
    ...
    revert x A0 C HeqD l1 l2 Heqll l pi1' ; induction pi1 ;
    ...
    * rnow simpl in IHpi1 then simpl in pi1.
      ...
      apply (rnpup 1) in pi1' ; rnow simpl in pi1' then simpl in pi1'.
      rewrite map_app in pi1' ; rewrite map_map in pi1'; rewrite (map_map _ _ l2) in pi1' ;
        rewrite 2 (map_ext _ _ (fup_fup_com _)) in pi1' ;
        rewrite <- map_map in pi1' ; rewrite <- (map_map _ _ l2) in pi1'...
  + eapply rexse.
