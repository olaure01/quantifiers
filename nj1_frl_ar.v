...
destruct (dichot_elt_app_inf _ _ _ _ _ H) as [ [? [? ?]] | [? [? ?]] ]; subst;
  rewrite ? (app_assoc _ _ (A::_)), <- ? (app_assoc _ (A::_)), <- ? app_comm_cons; intuition.
Qed.
...
Goal rprove nil
  (imp (frl x (fconstr (tconstr (tvar _ x) (tfun _ f)) (frel _ P)))
       (frl x (fconstr (tconstr (tconstr (tvar _ x) (tfun _ f))
                                                  (tfun _ f)) (frel _ P)))).
Proof.
rnow rev_intros.
